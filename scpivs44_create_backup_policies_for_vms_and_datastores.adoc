...
* 如果您想要VM一致的備份、則必須安裝並執行VMware Tools。需要VMware工具來停止虛擬機器。
...
NOTE: 此選項支援FlexGroup 從SnapCenter VMware vSphere 4.5版的VMware vSphere Plug-in開始的所有資料存放區。不支援FlexGroup 此選項、適用於SnapCenter VMware vSphere 4.4及更早版本的VMware vCenter Plug-in中的資料存放區不支援此選項。
