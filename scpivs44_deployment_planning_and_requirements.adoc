...
[role="lead"]
在部署虛擬應用裝置之前、您應該先瞭解部署需求。下列四個表格列出部署需求。
...
| 主要目的地 | 若要透過VMware SnapCenter VMware vCenter Standard執行以應用程式為基礎的保護、只能SnapRestore 執行VMware VM和資料存放區的保護功能、僅限使用VMware VMware vCenter：用於還原作業FlexClone：用於掛載和附加作業 


| 次要目的地 | 若要透過VMware SnapCenter VMware vCenter Standard執行應用程式型保護：用於容錯移轉作業、僅執行VMware VM和資料存放區的保護FlexClone：用於掛載和附加作業 
|===
...
| vCenter vSphere | HTML5用戶端：不支援6.5U2/U3、6.7x、7.0、7.0U1、7.0U2 Flex用戶端。 
...
| VMware vSphere MySQL資料庫適用的插件SnapCenter | MySQL 8.0.23 
...
|===
如需支援版本的最新資訊、請參閱 https://mysupport.netapp.com/matrix/imt.jsp?components=91324;&solution=1517&isHWU&src=IMT["NetApp 互通性對照表工具"^]。
...
| VMware vSphere連接埠適用的外掛程式SnapCenter | 8144（HTTPS）、雙向連接埠用於從VMware vSphere Web用戶端和SnapCenter 從VMware Server進行通訊。8080雙向此連接埠用於管理虛擬應用裝置。附註：您無法修改連接埠組態。 
